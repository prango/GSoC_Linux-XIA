...
int Hash_Value(int);
static inline int xt_to_vxt_rcu(int);
void resize();
void find_closer_bucket(struct xip_vxt_entry **,int *);
static inline int vxt_unregister_xidty(xid_type_t);
...
static inline int vxt_register_xidty(int key,xid_type_t xid_type,int reentrant){
...
static inline int vxt_unregister_xidty(xid_type_t key){
    int hash=key%XIP_VXT_TABLE_SIZE;
    struct xip_vxt_entry * start_bucket  = &B[hash];
    lock(start_bucket);
